...
def main(mlp: bool = True, debug: bool = False, use_tune_search: bool = True):
    arch = MLP() if mlp else CNN()
    dataset_root_dir = (
        "debug_dataset" if debug else "final_dataset_not_reduced"
    )
    ...
    (
        ...
        best_score,
        best_params,
    ) = utils.get_best_estimator_score_params(
        X_train,
        Y_train,
        X_valid,
        Y_valid,
    ...
    loss, acc = best_estimator.model_.evaluate(X_test, Y_test)
    utils.decorated_print("Evaluation results:")
    print("- loss:", loss)
    print("- accuracy:", acc)

    utils.decorated_print("Predicting test images")
    pred = best_estimator.model_.predict(X_test)
    print(
        "Predicted type for first test image:",
