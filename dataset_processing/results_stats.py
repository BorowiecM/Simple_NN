...
def plot_age_stats(hist_data):
    ...
    fig, axes = plt.subplots(
        nrows=nrows, ncols=ncols, figsize=(9, 6), sharey=True
    )
...
def count_images_in_folder(folder):
    ...
    if len(directories) == 0:
        ...
        results = {}
        directories_full_paths = [
            os.path.join(folder, directory) for directory in directories
        ]
        for i in range(len(directories)):
            results[directories[i]] = len(
                [
                    name
                    for name in os.listdir(directories_full_paths[i])
                    if os.path.isfile(
                        os.path.join(directories_full_paths[i], name)
                    )
                ]
            )
        return results
...
def display_animals_column_plots():
    ...
    recognised_animals = count_images_in_folder(
        "./results/datasets/animals_10"
    )
    chimpanzees = count_images_in_folder("./datasets/chimpanzees")
    recognised_chimpanzees = count_images_in_folder(
        "./results/datasets/chimpanzees"
    )
    ...
    fig.suptitle(
        "Comparison of initial datasets count vs recognised faces",
        fontweight="bold",
        fontsize=18,
    )
    ...
    axes[0].bar(
        br1,
        list(animals.values()) + list(chimpanzees.values()),
        color="b",
        width=barWidth,
        edgecolor="grey",
    ...
    axes[0].bar(
        br2,
        list(recognised_animals.values())
        + list(recognised_chimpanzees.values()),
        color="y",
        width=barWidth,
        edgecolor="grey",
    ...
    axes[0].set_xticks(
        [r + (0.5 * barWidth) for r in range(len(animals) + len(chimpanzees))],
    ...
    axes[1].set_xticks(
        [r + (0.5 * barWidth) for r in range(len(monkeys))], monkeys.keys()
    )
    axes[1].legend()
