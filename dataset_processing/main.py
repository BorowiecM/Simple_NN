...
class DatasetProcessor:
    ...
    def detect_and_save(self, item):
        ...
        try:
            if not os.path.exists(result_file) and item.endswith((".jpg", ".jpeg", ".png")) and not item.startswith("./results"):
                faces = self.detect_photos(item)
                if len(faces) > 0:
                    self.process_faces(item, result_folder, faces)
                print("Processed file " + item)
        except Exception as e:
            print('File \"' + os.path.split(item)[1] + '\" caused exception: ' + str(e))
    ...
    def process_faces(self, image_path, results_path, faces):
        with Image.open(image_path) as image:
            output_path = os.path.join(results_path, os.path.split(image_path)[1])
    ...
    def draw_faces(self, image, faces):
        ...
        for face in faces:
            ...
            image_draw.rectangle([x, y, (width + x), (y + height)], fill = None, outline = "red")
        return image
    ...
    if not os.path.exists(filename):
        with open(filename, "w+") as folders_file:
            for root, dirs, files in os.walk("."):
                for name in files:
                    if name.endswith((".jpg", ".jpeg", ".png")) and not root.startswith("./results"):
...
def parse_arguments():
    ...
    parser.add_argument('--crop', action='store_true', 
                        help='indicate that largest detected face will be cropped from image, instead of marking detected faces.')
